...
//레이지 아츠
final List rageArts = ["Radiant Pegasus Bomb", "레이지 상태에서 ${main.sticks["c3"]}AP", "20", "-15", "D", "D", "중단", "55", "혹은 레이지 상태에서 디스트럭티브 폼 도중 ${main.sticks["c3"]}AP\n레이지 아츠\n히트 시 상대의 회복 가능 게이지를 없앰\n홀드 가능"];
...
List types = [ //변경해야될것
  {"heat" : true}, {"general" : true}, {"sit" : true}, {"destructive form" : true}, {"boot" : true}, {"dual boot" : true}, {"backup" : true}
];

Map<String, String> typesKo = {
  "heat" : "히트", "general" : "일반", "sit" : "앉은 자세", "destructive form" : "디스트럭티브 폼", "boot" : "부트", "dual boot" : "듀얼 부트", "backup" : "백업"
};
      ...
      return Expanded(
        ...
        child: Padding(
          ...
          child: SizedBox(
            ...
            }, child: Text(content, style: const TextStyle(color: Colors.white, fontSize: main.keyboardFontSize),)),
          ),
        ),
      );
    }else if(content == "AC"){
...
TextStyle commandStyle = const TextStyle(fontWeight: FontWeight.w700, fontSize: 15),
        ...
        child: Column(
            ...
            children: [
              ...
              if(heatSystemMenu == true) // 히트 시스템 설명
                SizedBox(child: heatSystemContexts(["체인톱을 사용하는 공격의 가드 대미지 증가", "새로운 Dual Boot 이행기 사용 가능"])), //변경해야될것
                ...
                child: DataTable(
                    ...
                    rows: [
                      if(_searchText.isEmpty || rageArts.toString().toLowerCase().contains(_searchText.toLowerCase()))
                        DataRow(color: MaterialStateColor.resolveWith((states) => const Color(0xffd5d5d5)) ,cells : (createCommand(rageArts[0], rageArts[1], rageArts[2], rageArts[3], rageArts[4], rageArts[5], rageArts[6], rageArts[7], rageArts[8]))), //레이지 아츠
                      for(int i = 0; i < types.length; i++)...[
                        if(types[i][filtered[i]["type"]] == true)...[
                          for(int j = 0; j < filtered[i]["contents"].length; j ++)...[
                            if(listLength % 2 == 1)...[
        ...
        child: DataTable(
          ...
          rows: [
            for(int i = 0; i < widget.throws.length; i++)...[
              if(i % 2 == 0)...[
                DataRow(cells: createThrow(widget.throws[i][0], widget.throws[i][1], widget.throws[i][2], widget.throws[i][3], widget.throws[i][4], widget.throws[i][5], widget.throws[i][6], widget.throws[i][7]), color: MaterialStateColor.resolveWith((states) => const Color(0xffd5d5d5)))
              ]else...[
