...
class JIN extends StatefulWidget {
  ...
  const JIN({super.key, required this.commands, required this.throws});
...
class _JINState extends State<JIN> {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      theme: ThemeData(fontFamily: 'Tenada', useMaterial3: false),
                ...
                child: Column(
                  children: [
                    ...
                    Padding(
                      ...
                      child: TextField(controller: text, decoration: InputDecoration(labelStyle: TextStyle(color: Colors.white), label: Text("검색"), border: OutlineInputBorder()), style: TextStyle(color: Colors.white), onChanged: (value) {
                        setState((){
                          searchText = value;
                        });
                      },),
            ...
            Column(
              ...
              children: [
                ...
                if(heatSystemMenu == true) // 히트 시스템 설명
                  SizedBox(child: heatSystemContexts(["데빌의 힘을 사용하는 새로운 기술 사용 가능", "최속 입력이 아니더라도 Electric Thrusting Uppercut과 Electric Wind Hook Fist 사용 가능"])),
                SingleChildScrollView(
                  ...
                  child: DataTable(
                    ...
                    rows: [
                      if(searchText.isEmpty || "Polar Demon Gouge ${main.sticks["c3"]}AP 20 -15 D D 중단 55 레이지 아츠\n히트 시 상대의 회복 가능 게이지를 없앰".toString().toLowerCase().contains(searchText.toLowerCase()))
                        DataRow(cells : (creatCommand("Polar Demon Gouge", "${main.sticks["c3"]}AP", "20", "-15", "D", "D", "중단", "55", "레이지 아츠\n히트 시 상대의 회복 가능 게이지를 없앰"))),
                      for(int j = 0; j < types.length; j++)...[
...
List<DataCell> createThrow(String name, command, start, breakThrow, frameAfterBreak, damage, range, extra){
  return [
    ...
    DataCell(SizedBox(width: 50, child: Text(damage, textAlign: TextAlign.center, textScaler: scale, style: commandStyle))), //대미지
    DataCell(SizedBox(width: 30, child: Text(range, textAlign: TextAlign.center, textScaler: scale, style: commandStyle))), //판정
    DataCell(Text(extra, textAlign: TextAlign.start, textScaler: scale, style: commandStyle)), //비고
  ];
}
