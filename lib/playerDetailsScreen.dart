...
class PlayerDetails{
  ...
  PlayerDetails({required this.stats, required this.prowess, required this.imgUrl, required this.rankPoint});
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text(widget.playerInfo.name),
...
class _StatTabState extends State<StatTab> with AutomaticKeepAliveClientMixin{
  ...
  Future<PlayerDetails> details()async{
    ...
    final parse = Uri.parse(widget.playerInfo.url);
    ...
    PlayerDetails playerDetails = PlayerDetails(stats: statMap, prowess: prowess, imgUrl: imgUrl.toString(), rankPoint: rankPoint);
                  ...
                  children: [
                    Row(
                      ...
                      children: [
                        ...
                        headingBox(20, "승", "timeWin"),
                        headingBox(20, "패", "timeLose"),
                            ...
                            return SizedBox(
                              ...
                              child: Row(
                                ...
                                children: [
                                  ...
                                  SizedBox(width: 60, child: Text(data[index].timeFaced, textAlign: TextAlign.center)),
                                  SizedBox(width: 20, child: Text(data[index].timeWin, textAlign: TextAlign.center)),
                                  SizedBox(width: 20, child: Text(data[index].timeLose, textAlign: TextAlign.center)),
...
class _HistoryTabState extends State<HistoryTab> with AutomaticKeepAliveClientMixin{
  ...
  String convertDate(String date){
    ...
    final String month = forward.split(" ")[0], day = forward.split(" ")[1];
    final String year = backward.split('@')[0], time = backward.split('@')[1];

    //forward = 04 02, year = 2024, time = 03:30
    final String convertedDate = "$year년 $month월 $day일 $time";
  ...
  Future<List<PlayHistory>> historyFuture()async{
    ...
    final url = widget.playerInfo.url;
    final parse = Uri.parse(url);
    final response = await http.get(parse);
    dom.Document document = parser.parse(response.body);
