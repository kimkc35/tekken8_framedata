...
class _MainState extends State<Main> {
  ...
  void initState() {
    super.initState();
    for(var types in widget.moves){
      for(var contents in types["contents"]){
        for(int i = 0; i < extraInitials.length; i++){
          contents[8] = contents[8].toString().replaceAll(extraInitials[i]["name"].toString(), extraInitials[i][extraInitials[i]["name"]].toString()).replaceAll("/", "\n");
        }
      }
    }
  }
