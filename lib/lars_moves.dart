...
//레이지 아츠
final List rageArts = ["제우스 언리미티드", "레이지 상태에서 ${sticks["c3"]}AP", "20", "-15", "D", "D", "중단", "55", "레이지 아츠\n히트 시 상대의 회복 가능 게이지를 없앰"];
...
List<String> heatSystem = ["파워가 상승한 리미티드 엔트리 사용 가능", "새로운 사일런트 엔트리 이행기 사용 가능"];
const character = "lars"; //변경해야될것

List types = [ //변경해야될것
  {"heat" : false}, {"general" : false}, {"sit" : false}, {"dynamic entry" : false}, {"silent entry" : false}, {"limited entry" : false}
];
    ...
    if(content == "delete"){
      return Expanded(
        ...
        child: Padding(
          ...
          child: SizedBox(
            height: 40,
            child: TextButton(style: ButtonStyle(side: WidgetStateBorderSide.resolveWith((states) => BorderSide(color: Colors.pink))),onPressed: (){
              setState(() {
