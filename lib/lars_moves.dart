...
//레이지 아츠
final List rageArts = ["Zeus Unlimited", "레이지 상태에서 ${sticks["c3"]}AP", "20", "-15", "D", "D", "중단", "55", "레이지 아츠\n히트 시 상대의 회복 가능 게이지를 없앰"];
...
List types = [ //변경해야될것
  {"heat" : true}, {"general" : true}, {"sit" : true}, {"dynamic entry" : true}, {"silent entry" : true}, {"limited entry" : true}
];
      ...
      return Expanded(
        ...
        child: Padding(
          ...
          child: SizedBox(
            ...
            }, child: Text(content, style: TextStyle(color: Colors.white, fontSize: keyboardFontSize),)),
          ),
        ),
      );
    }else if(content == "AC"){
        ...
        child: Column(
            ...
            children: [
              ...
              if(heatSystemMenu == true) // 히트 시스템 설명
                SizedBox(child: heatSystemContexts(["파워가 상승한 Limited Entry 사용 가능", "새로운 Silent Entry 이행기 사용 가능"])), //변경해야될것
                ...
                child: DataTable(
                    ...
                    rows: [
                      if(_searchText.isEmpty || rageArts.toString().toLowerCase().contains(_searchText.toLowerCase()))
                        DataRow(color: MaterialStateColor.resolveWith((states) => Color(0xffd5d5d5)) ,cells : (createMove(context, character, rageArts[0], rageArts[1], rageArts[2], rageArts[3], rageArts[4], rageArts[5], rageArts[6], rageArts[7], rageArts[8]))), //레이지 아츠
                      for(int i = 0; i < types.length; i++)...[
        ...
        child: DataTable(
          ...
          rows: [
            for(int i = 0; i < widget.throws.length; i++)...[
              if(i % 2 == 0)...[
                DataRow(cells: createThrow(context, character, widget.throws[i][0], widget.throws[i][1], widget.throws[i][2], widget.throws[i][3], widget.throws[i][4], widget.throws[i][5], widget.throws[i][6], widget.throws[i][7]), color: MaterialStateColor.resolveWith((states) => Color(0xffd5d5d5)))
              ]else...[
